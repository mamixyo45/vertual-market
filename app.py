...
BIST30_ORNEK = ["AKBNK","ASELS","BIMAS","EKGYO","ENKAI","EREGL","FROTO","GARAN","HEKTS","ISCTR",
                "KCHOL","KONTR","KOZAL","KRDMD","MGROS","ODAS","OYAKC","PETKM","PGSUS","SAHOL",
                "SASA","SISE","TAVHL","TCELL","THYAO","TOASO","TUPRS","TTKOM","ULKER","YKBNK"]

tum_kodlar = kap_tum_liste() or sorted(u[0] for u in UNIVERSE)

# --- marka + sol menü + içerik düzeni ---
st.markdown("<div style='display:flex;align-items:center;gap:12px;padding:4px 2px 10px'>"
            "<span style='font-size:26px'>◈</span>"
            "<span class='vm-brand'>VERTUAL MARKET</span></div>", unsafe_allow_html=True)
...
with sagc:
    ...
    if sym not in tum_kodlar:
        sym = tum_kodlar[0]
    grup = st.session_state.get("grup", "Tümü")

    if view in ("piyasa", "bot"):
        k1, k2 = st.columns([2, 1]) if view == "piyasa" else (st.container(), None)
        with (k1 if view == "piyasa" else k1):
            sym = st.selectbox("Hisse Seç (yazarak arayabilirsin)", tum_kodlar,
                               index=tum_kodlar.index(sym),
                               format_func=lambda k: f"{k} — {ISIM_SOZLUGU.get(k, '')}".rstrip(" —"))
            st.session_state["sym"] = sym
        if view == "piyasa":
            with k2:
                grup = st.selectbox("Liste / Endeks", ["Tümü", "◈ Katılım (örnek)",
                                                       "BIST 30 (örnek)", "Popüler 100"])
    ...
    if view == "ayarlar":
        with st.expander("⚙️ Portföy Ayarları", expanded=True):
            ...
            yeni_b = a1.number_input("Sanal başlangıç bakiyesi (₺)", 100.0, 10_000_000.0,
                                     G["baslangic"], step=100.0)
            if a2.button("♻️ Portföyü Sıfırla", use_container_width=True):
    ...
    temel = temel_bilgi(tuple(sorted(u[0] for u in UNIVERSE))) if view == "piyasa" else {}
    SEKTOR = {u[0]: u[3] for u in UNIVERSE}

    # grup filtresi (tablo için)
    if grup == "◈ Katılım (örnek)":
        liste_kodlar = [k for k in tum_kodlar if katilim_etiketi(k) == "uygun"]
    elif grup == "BIST 30 (örnek)":
        liste_kodlar = [k for k in tum_kodlar if k in BIST30_ORNEK]
    elif grup == "Popüler 100":
        liste_kodlar = [k for k in tum_kodlar if k in ISIM_SOZLUGU]
    else:
    ...
    payload = {
        ...
        "stocks": [{"k": k, "n": ISIM_SOZLUGU.get(k, ""), "katd": katilim_etiketi(k),
                    ...
                    "sec": SEKTOR.get(k)} for k in liste_kodlar] if view == "piyasa" else [],
        "haberler": hbr,
...
<div id="v_piyasa" style="__D_piyasa__">
  <div class="card">
    <div class="h"><span id="pg_baslik"></span><span id="pg_fiyat" class="fbig" style="margin-left:6px"></span>
      <span id="pg_pct" style="font-size:14px"></span>
      <span class="mut" style="margin-left:auto" id="t_xu"></span></div>
  ...
  <div class="card"><div class="h"><span id="liste_baslik">Hisse Listesi</span>
    ...
    resmî listeden doğrulayın. Analist (AI) kural tabanlı otomatik derecelendirmedir; yatırım tavsiyesi değildir.</div></div>
</div>
...
document.getElementById("liste_baslik").textContent="Hisse Listesi · "+D.grup+" ("+D.stocks.length+")";
