...
<div class="wrap">
  <div class="nav">
    <a data-v="piyasa" onclick="nav('piyasa')" class="__ON_piyasa__">📈 Piyasa</a>
    <a data-v="bot" onclick="nav('bot')" class="__ON_bot__">🤖 Bot</a>
    <a data-v="portfoy" onclick="nav('portfoy')" class="__ON_portfoy__">💼 Portföy</a>
    <a data-v="islemler" onclick="nav('islemler')" class="__ON_islemler__">🧾 İşlemler</a>
    <a data-v="ayarlar" onclick="nav('ayarlar')" class="__ON_ayarlar__">⚙️ Ayarlar</a>
  </div>
  <div class="mid">
    ...
    <div id="v_piyasa" style="__D_piyasa__">
      <div class="card"><div class="h"><span id="pg_baslik"></span>
        <span style="margin-left:auto;display:flex;gap:5px;flex-wrap:wrap" id="tfbtns"></span></div>
  ...
  <div class="right" id="right" style="__D_right__">
    <div class="card"><div class="h">🤖 Otomatik Al-Sat Botu <span class="mut">(SANAL)</span></div>
      <div class="kv"><span>Durum</span>
        ...
        <span class="sw" id="botsw" onclick="toggleBot()"><i></i></span></span></div>
      ...
      <div class="warn">Bot sanal parayla işlem yapar, gerçek emir göndermez.</div></div>
  </div>
</div>
<script>
...
const url=window.location.pathname.replace(/\/~\/.*/,"/")+"?"+q.toString();
try{window.top.location.href="?"+q.toString();return;}catch(e){}
try{window.parent.location.href="?"+q.toString();return;}catch(e){}
const a=document.createElement("a");a.href="?"+q.toString();a.target="_top";
...
show("v_islemler",v==="islemler");show("v_ayarlar",v==="ayarlar");show("right",v==="bot");
...
"<div class='mut' style='padding:40px;text-align:center'>Bu zaman dilimi için veri yok.</div>";return;}
if(!window.LightweightCharts){kutu.innerHTML=
"<div class='mut' style='padding:40px;text-align:center'>Grafik kütüphanesi yüklenemedi; sayfayı yenile.</div>";return;}
...
height:kutu.clientHeight||400,width:kutu.clientWidth||undefined,
...
// piyasa grafiği + zaman dilimi düğmeleri (sayfa yenilenmeden değişir)
let aktifTf=D.tford.includes("1d")?"1d":(D.tford[0]||null);
function cizP(k){if(!k)return;aktifTf=k;
document.querySelectorAll(".tfb").forEach(b=>b.classList.toggle("on",b.dataset.k===k));
document.getElementById("pg_baslik").textContent=D.sym+" · "+(D.tfdata[k]?D.tfdata[k].ad:"")+" Grafik";
grafik("chart_p",D.tfdata[k],false);}
const tfb=document.getElementById("tfbtns");
D.tford.forEach(k=>{const b=document.createElement("button");
b.className="btn tfb";b.dataset.k=k;b.textContent=D.tfdata[k].ad;
...
tr.innerHTML="<td><b>"+s.k+"</b></td><td class='mut'>"+s.n+"</td><td>"+katHtml(s.katd)+"</td>"+
...
window.resetPort=resetPort;window.saveIzleme=saveIzleme;window.nav=nav;
// başlangıç çizimleri
if(D.view==="piyasa")cizP(aktifTf);
if(D.view==="bot")cizB();
</script></body></html>"""

# görünürlük ve aktif menüyü sunucu tarafında bas (JS çökse bile doğru sayfa görünür)
