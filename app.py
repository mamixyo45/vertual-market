...
# Zaman dilimleri: (anahtar, etiket, periyot, aralık, yeniden örnekleme)
...
def yeniden_ornekle(df, kural):
    o = df.resample(kural).agg({"open": "first", "high": "max", "low": "min",
...
if view not in ("piyasa", "bot", "portfoy", "islemler", "ayarlar"): view = "piyasa"
mesaj = ""
...
if any(k in qp for k in ("bot", "risk", "tutar", "reset", "bakiye", "tara", "izleme")):
    ...
    st.query_params.update({"sym": sym, "view": view})
    st.rerun()
...
font-size:14.5px;margin-bottom:4px;cursor:pointer;text-decoration:none}
...
padding:10px 14px;cursor:pointer;font-size:13px;text-decoration:none;display:inline-block;text-align:center}
...
.risk{display:flex;gap:6px}.risk .btn{flex:1;padding:8px 4px}
...
.kv{display:flex;justify-content:space-between;padding:8px 0;font-size:13.5px;border-bottom:1px dashed var(--line);align-items:center}
...
.kodlink{color:var(--gold);text-decoration:none;font-weight:700}
...
<div class="wrap">
  <div class="nav">
    <a href="/?view=piyasa&sym=__SYM__" target="_top" class="__ON_piyasa__">📈 Piyasa</a>
    <a href="/?view=bot&sym=__SYM__" target="_top" class="__ON_bot__">🤖 Bot</a>
    <a href="/?view=portfoy&sym=__SYM__" target="_top" class="__ON_portfoy__">💼 Portföy</a>
    <a href="/?view=islemler&sym=__SYM__" target="_top" class="__ON_islemler__">🧾 İşlemler</a>
    <a href="/?view=ayarlar&sym=__SYM__" target="_top" class="__ON_ayarlar__">⚙️ Ayarlar</a>
  </div>
  <div class="mid">
    <div id="jshata"></div><div id="mesaj"></div>

    <div id="v_piyasa" style="__D_piyasa__">
      <div class="card">
        <div class="h"><span id="pg_baslik"></span>
          <form action="/" method="get" target="_top" style="display:flex;gap:6px;margin-left:auto">
            <input type="hidden" name="view" value="piyasa">
            <input name="sym" list="dl" placeholder="Hisse kodu yaz..." style="width:150px" autocomplete="off">
            <datalist id="dl"></datalist>
            <button class="btn" type="submit">Göster</button></form></div>
        <div class="h" style="margin-top:2px">
          <span style="display:flex;gap:5px;flex-wrap:wrap" id="tfbtns"></span>
          <span style="margin-left:auto;display:flex;gap:5px">
            <button class="btn tfb" id="d_trend" onclick="cizimMod('trend')">✏️ Trend</button>
            <button class="btn tfb" id="d_yatay" onclick="cizimMod('yatay')">─ Yatay</button>
            <button class="btn tfb" onclick="cizimSil()">🗑 Temizle</button></span></div>
      ...
      <div class="card"><div class="h">📰 Son Dakika Piyasa Haberleri</div><div id="haberler"></div>
        <div class="warn">Kaynaklar: Bloomberg HT / AA Ekonomi / Dünya (RSS).</div></div>
      <div class="card"><div class="h">Hisse Listesi
        <input id="ara" placeholder="Hisse ara..." style="max-width:300px;margin-left:auto">
        ...
        <div class="warn">Koda tıklayınca o hisse açılır. Katılım etiketleri örnektir; resmî listeden doğrulayın.</div></div>
    </div>
    ...
    <div id="v_ayarlar" class="card" style="__D_ayarlar__"><div class="h">⚙️ Ayarlar</div>
      <div class="kv"><span>Sanal başlangıç bakiyesi (₺)</span></div>
      <form action="/" method="get" target="_top" style="display:flex;gap:8px;margin:8px 0">
        <input type="hidden" name="view" value="ayarlar"><input type="hidden" name="sym" value="__SYM__">
        <input type="hidden" name="reset" value="1">
        <input name="bakiye" type="number" min="100" value="__BAK__" style="flex:1">
        <button class="btn r" type="submit" onclick="return confirm('Sanal portföy sıfırlansın mı?')">Portföyü Sıfırla</button></form>
      <div class="kv"><span>Botun izlediği hisseler (virgülle, en çok 15)</span></div>
      <form action="/" method="get" target="_top" style="display:flex;gap:8px;margin:8px 0">
        <input type="hidden" name="view" value="ayarlar"><input type="hidden" name="sym" value="__SYM__">
        <input name="izleme" value="__IZL__" style="flex:1">
  ...
  <div class="right" id="right" style="__D_right__">
    <div class="card"><div class="h">🤖 Otomatik Al-Sat Botu <span class="mut">(SANAL)</span></div>
      <div class="kv"><span>Durum</span>
        <span style="display:flex;align-items:center;gap:8px"><span id="b_durum"></span>
        <a class="sw __SWON__" href="/?view=bot&sym=__SYM__&bot=__BOTTERS__" target="_top"><i></i></a></span></div>
      <div class="kv"><span>Strateji</span><span>Sinyal Skoru</span></div>
      <div style="margin:10px 0 4px" class="mut">Yatırım Tutarı (sanal ₺)</div>
      <form action="/" method="get" target="_top" style="display:flex;gap:8px">
        <input type="hidden" name="view" value="bot"><input type="hidden" name="sym" value="__SYM__">
        <input name="tutar" type="number" min="50" value="__TUT__" style="flex:1">
        <button class="btn" type="submit">✓</button></form>
      <div style="margin:12px 0 6px" class="mut">Risk Seviyesi</div>
      <div class="risk">
        <a class="btn __R_Düşük__" href="/?view=bot&sym=__SYM__&risk=Düşük" target="_top">Düşük</a>
        <a class="btn __R_Orta__" href="/?view=bot&sym=__SYM__&risk=Orta" target="_top">Orta</a>
        <a class="btn __R_Yüksek__" href="/?view=bot&sym=__SYM__&risk=Yüksek" target="_top">Yüksek</a></div>
      ...
      <div style="display:flex;gap:8px;margin-top:14px">
        <a class="btn r" style="flex:1" href="/?view=bot&sym=__SYM__&bot=__BOTTERS__" target="_top">__BOTBTN__</a>
        <a class="btn p" style="flex:1" href="/?view=bot&sym=__SYM__&tara=1" target="_top">🔍 Şimdi Tara</a></div>
      <div class="warn">Bot sanal parayla işlem yapar, gerçek emir göndermez. "Şimdi Tara"
      izleme listesini tarar ve sinyale göre sanal AL/SAT yapar (bot açıkken).</div></div>
  </div>
</div>
<script>
...
:kd==="disi"?'<span class="red">✖ Uygun Değil</span>':'<span class="mut">? Doğrulanmadı</span>';}
document.getElementById("t_fiyat").textContent=fmt(D.fiyat)+" ₺";
document.getElementById("t_pct").innerHTML=pctHtml(D.pct);
document.getElementById("t_ad").innerHTML="<b>"+D.sym+"</b> · "+(D.isim||"")+" · "+katHtml(D.katd);
...
// sembol öneri listesi
const dl=document.getElementById("dl");
D.stocks.forEach(s=>{const o=document.createElement("option");o.value=s.k;o.label=s.n;dl.appendChild(o);});
// grafik motoru
let GP=null;
function grafik(el,veri,sr){const kutu=document.getElementById(el);kutu.innerHTML="";
if(!veri||!veri.bars||!veri.bars.length){kutu.innerHTML=
"<div class='mut' style='padding:40px;text-align:center'>Bu zaman dilimi için veri yok.</div>";return null;}
...
if(veri.ma20&&veri.ma20.length)ch.addLineSeries({color:"#2EBD85",lineWidth:1}).setData(veri.ma20);
if(veri.ma50&&veri.ma50.length)ch.addLineSeries({color:"#F6465D",lineWidth:1}).setData(veri.ma50);
...
function cizimMod(m){mod=(mod===m)?null:m;ilk=null;
...
if(mod==="yatay"){yataylar.push(GP.cs.createPriceLine({price:fiyat,color:"#E4B95B",
lineWidth:1,title:fiyat.toFixed(2)}));cizimMod("yatay");cizimMod(null===mod?null:null);mod=null;
document.getElementById("d_yatay").classList.remove("on");}
else if(mod==="trend"){
if(!ilk){ilk={time:param.time,value:fiyat};}
else{let a=ilk,b={time:param.time,value:fiyat};
if((typeof a.time==="number"&&a.time>b.time)||(typeof a.time==="string"&&a.time>b.time)){const t=a;a=b;b=t;}
if(a.time!==b.time){const ls=GP.ch.addLineSeries({color:"#E4B95B",lineWidth:2});
ls.setData([a,b]);cizgiler.push(ls);}
ilk=null;mod=null;document.getElementById("d_trend").classList.remove("on");}}});}
// piyasa grafiği + zaman dilimleri
...
document.getElementById("pg_baslik").textContent=D.sym+" · "+(D.tfdata[k]?D.tfdata[k].ad:"")+" Grafik";
GP=grafik("chart_p",D.tfdata[k],false);cizimBagla();}
...
b.onclick=()=>cizP(k);tfb.appendChild(b);});
function cizB(){document.getElementById("bg_baslik").textContent=
D.sym+" · Bot Analiz Grafiği (Günlük) · Destek "+(D.destek??"—")+" / Direnç "+(D.direnc??"—");
grafik("chart_b",{bars:D.barsB,ma20:D.maB20,ma50:D.maB50},true);}
// haberler
...
// hisse tablosu (kod = gerçek bağlantı)
...
tr.innerHTML='<td><a class="kodlink" target="_top" href="/?view=piyasa&sym='+s.k+'">'+s.k+'</a></td>'+
...
window.cizimMod=cizimMod;window.cizimSil=cizimSil;
// başlangıç çizimleri
if(D.view==="piyasa")cizP(aktifTf);
if(D.view==="bot")cizB();
</script></body></html>"""

# --- sunucu tarafı yer tutucular ---
for v in ("piyasa", "bot", "portfoy", "islemler", "ayarlar"):
    HTML = HTML.replace(f"__D_{v}__", "" if view == v else "display:none")
    HTML = HTML.replace(f"__ON_{v}__", "on" if view == v else "")
HTML = HTML.replace("__D_right__", "" if view == "bot" else "display:none")
HTML = HTML.replace("__SYM__", sym)
HTML = HTML.replace("__BOTTERS__", "0" if G["aktif"] else "1")
HTML = HTML.replace("__BOTBTN__", "Durdur" if G["aktif"] else "Başlat")
HTML = HTML.replace("__SWON__", "on" if G["aktif"] else "")
HTML = HTML.replace("__TUT__", str(int(G["tutar"])))
HTML = HTML.replace("__BAK__", str(int(G["baslangic"])))
HTML = HTML.replace("__IZL__", ", ".join(G["izleme"]))
for r in ("Düşük", "Orta", "Yüksek"):
    HTML = HTML.replace(f"__R_{r}__", "on" if G["risk"] == r else "")
