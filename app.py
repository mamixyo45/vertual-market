...
@st.cache_data(ttl=86400, show_spinner=False)
def temel_bilgi(kodlar):
    """Piyasa değeri ve F/K — zaman bütçeli (ilk yüklemede en fazla ~45 sn)."""
    out = {}
    baslangic = _time.time()
    for k in kodlar:
        if _time.time() - baslangic > 45:
            break
        try:
            info = yf.Ticker(k + ".IS").info
            mc = info.get("marketCap")
            fk = info.get("trailingPE")
            out[k] = {"mc": round(mc / 1e9, 2) if mc else None,
...
.block-container{padding:0.3rem 0.5rem;max-width:100%}
...
# --- ÜST KONTROL ŞERİDİ (platform kısıtı gereği bu iki kontrol gömülü ekranda çalışamaz) ---
ks1, ks2 = st.columns([2, 3])
with ks1:
    varsayilan = st.session_state.get("sym", "THYAO")
    if varsayilan not in tum_kodlar:
        varsayilan = tum_kodlar[0]
    sym = st.selectbox("Hisse Seç (yazarak arayabilirsin)", tum_kodlar,
                       index=tum_kodlar.index(varsayilan),
                       format_func=lambda k: f"{k} — {ISIM_SOZLUGU.get(k, '')}".rstrip(" —"))
    st.session_state["sym"] = sym
with ks2:
    with st.expander("⚡ Bot Kontrolleri (Başlat · Tara · Risk · Ayarlar)"):
        b1, b2, b3 = st.columns([1, 1, 1])
        G["aktif"] = b1.toggle("Bot Aktif (sanal)", value=G["aktif"])
        G["risk"] = b2.radio("Risk", ["Düşük", "Orta", "Yüksek"],
                             index=["Düşük", "Orta", "Yüksek"].index(G["risk"]), horizontal=True)
        G["tutar"] = b3.number_input("İşlem tutarı ₺", 50.0, 100000.0, G["tutar"], step=50.0)
        G["izleme"] = st.multiselect("Botun izlediği hisseler (en çok 15)", tum_kodlar,
                                     default=[k for k in G["izleme"] if k in tum_kodlar],
                                     max_selections=15)
        t1, t2, t3 = st.columns([1, 1, 1])
        if t1.button("🔍 Şimdi Tara", type="primary", use_container_width=True):
            if not G["aktif"]:
        ...
        yeni_b = t2.number_input("Başlangıç ₺", 100.0, 10_000_000.0, G["baslangic"], step=100.0)
        if t3.button("♻️ Portföyü Sıfırla", use_container_width=True):
            G.update({"bakiye": yeni_b, "baslangic": yeni_b, "poz": {}, "gunluk": []})
...
with st.spinner(f"{len(tum_kodlar)} hissenin verisi yükleniyor (ilk açılış 1-2 dk sürebilir)..."):
    piyasa = toplu_veri(tuple(tum_kodlar))
    temel = temel_bilgi(tuple(sorted(u[0] for u in UNIVERSE)))
...
payload = {
    ...
    "stocks": [{"k": k, "n": ISIM_SOZLUGU.get(k, ""), "katd": katilim_etiketi(k),
                ...
                "sec": SEKTOR.get(k)} for k in tum_kodlar],
    "haberler": hbr,
...
.top{display:flex;align-items:center;gap:16px;padding:12px 18px;background:var(--card);
border-bottom:1px solid var(--line);flex-wrap:wrap}
.brand{font-family:Cinzel,serif;font-size:24px;font-weight:700;letter-spacing:3px;
background:linear-gradient(135deg,#F7E2A5,#E4B95B 45%,#B98730);
-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent;
text-shadow:0 0 24px rgba(228,185,91,.25)}
.mut{color:var(--mut);font-size:13px}
.wrap{display:flex;min-height:820px}
...
font-size:14.5px;margin-bottom:4px;cursor:pointer;text-decoration:none;user-select:none}
...
<div class="top">
  <div style="display:flex;align-items:center;gap:12px">
    <svg width="42" height="42" viewBox="0 0 44 44" style="filter:drop-shadow(0 0 8px rgba(228,185,91,.45))">
  ...
  <span class="mut" style="margin-left:auto" id="t_xu"></span>
</div>
<div class="wrap">
  <div class="nav">
    <a data-v="piyasa">📈 Piyasa</a>
    <a data-v="bot">🤖 Bot</a>
    <a data-v="portfoy">💼 Portföy</a>
    <a data-v="islemler">🧾 İşlemler</a>
    <a data-v="ayarlar">⚙️ Ayarlar</a>
  </div>
  <div class="mid">
    <div id="jshata"></div>
    ...
    <div id="v_piyasa">
      <div class="card">
        ...
        <div class="warn">Hisse değiştirmek için sayfanın en üstündeki "Hisse Seç" kutusunu kullan.
        ✏️ Trend: grafikte iki noktaya dokun · ─ Yatay: bir noktaya dokun · Zaman dilimi değişince çizimler temizlenir.</div></div>
      ...
      <div class="card"><div class="h">Hisse Listesi
        ...
        <div class="warn">Hisse açmak için üstteki "Hisse Seç" kutusunu kullan. Katılım etiketleri örnektir;
        resmî listeden doğrulayın. Piyasa değeri ve F/K bazı hisselerde kademeli yüklenir. Analist (AI)
        sütunu kural tabanlı otomatik derecelendirmedir; yatırım tavsiyesi değildir.</div></div>
    </div>

    <div id="v_bot" style="display:none">
      <div class="card"><div class="h" id="bg_baslik"></div><div id="chart_b" style="height:380px"></div>
        <div class="warn">Destek/direnç seviyeleri, MA çizgileri ve AL/SAT işaretleri botun kendi analizidir
        ve yalnızca bu ekranda görünür.</div></div>
    ...
    <div id="v_ayarlar" class="card" style="display:none"><div class="h">⚙️ Ayarlar ve Bilgi</div>
      ...
      <div class="warn" style="margin-top:12px">Bot başlatma, tarama, risk, tutar, izleme listesi ve portföy
      sıfırlama işlemleri sayfanın <b>en üstündeki ⚡ Bot Kontrolleri</b> bölümünden yapılır
      (platform güvenliği bu komutların gömülü ekrandan verilmesine izin vermiyor).<br><br>
  ...
  <div class="right" id="right" style="display:none">
    <div class="card"><div class="h">🤖 Otomatik Al-Sat Botu <span class="mut">(SANAL)</span></div>
      ...
      <div class="warn" style="margin-top:12px">Başlat / Durdur / Şimdi Tara ve diğer kontroller
      sayfanın <b>en üstündeki ⚡ Bot Kontrolleri</b> bölümündedir. Bot sanal parayla işlem yapar,
      gerçek emir göndermez.</div></div>
  </div>
</div>
<script>
...
:kd==="disi"?'<span class="red">✖ Değil</span>':'<span class="mut">?</span>';}
...
document.getElementById("t_xu").innerHTML=D.xu?("BIST 100&nbsp; <b>"+D.xu.toLocaleString("tr-TR")+"</b> "+pctHtml(D.xup)):"";
// menü (tamamen tarayıcı içi — sayfa yenilenmez)
function nav(v){document.querySelectorAll(".nav a").forEach(a=>a.classList.toggle("on",a.dataset.v===v));
["piyasa","bot","portfoy","islemler","ayarlar"].forEach(x=>{
document.getElementById("v_"+x).style.display=(x===v)?"":"none";});
document.getElementById("right").style.display=(v==="bot")?"":"none";
if(v==="piyasa")cizP(aktifTf);if(v==="bot")cizB();}
document.querySelectorAll(".nav a").forEach(a=>a.addEventListener("click",()=>nav(a.dataset.v)));
// grafik motoru
let GP=null;
function grafik(el,veri,sr,ma){const kutu=document.getElementById(el);kutu.innerHTML="";
...
cizgiler=[];yataylar=[];ilk=null;modAyarla(null);dInfo.textContent="Çizimler temizlendi";
...
D.sym+" — "+(D.isim||"")+" · Bot Analiz Grafiği (Günlük)";
