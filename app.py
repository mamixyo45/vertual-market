...
# ----------------------------------------------------------------
# Kalıcı sanal portföy (sunucu yeniden başlayana dek korunur)
# ----------------------------------------------------------------
@st.cache_resource
def durum():
...
st.markdown("<style>#MainMenu,header,footer{visibility:hidden}"
            ".block-container{padding:0.4rem 0.5rem;max-width:100%}</style>",
...
if "bot" in qp:
    G["aktif"] = qp["bot"] == "1"
if "risk" in qp and qp["risk"] in ESIK:
    G["risk"] = qp["risk"]
if "tutar" in qp:
...
if qp.get("reset") == "1":
    ...
    mesaj = f"Sanal portföy {b:.0f} ₺ ile sıfırlandı."
...
for k, p in G["poz"].items():
    g = poz_fiyat.get(k, (p["giris"], 0))[0]
    pozisyonlar.append({"k": k, "a": p["adet"], "giris": round(p["giris"], 2),
                        "guncel": round(g, 2), "kz": round((g - p["giris"]) * p["adet"], 2)})
kapali = [g2 for g2 in G["gunluk"] if g2["y"] == "SAT"]
karli = sum(1 for g2 in kapali if (g2["kz"] or 0) > 0)
poz_deger = sum(p["a"] * p["guncel"] for p in pozisyonlar)
toplam = G["bakiye"] + poz_deger

payload = {
    "sym": sym, "isim": ISIM_SOZLUGU.get(sym, ""), "kat": KATILIM_TR[katilim_etiketi(sym)],
    "view": view, "mesaj": mesaj,
    "xu": round(xu, 0) if xu else None, "xup": round(xup, 2),
    "fiyat": round(A["price"], 2) if A else fiyatlar.get(sym, (0, 0))[0],
    "pct": round((A["price"] / float(df["close"].iloc[-2]) - 1) * 100, 2) if A else 0,
    "yuksek": round(float(df["high"].iloc[-1]), 2) if A else None,
    "dusuk": round(float(df["low"].iloc[-1]), 2) if A else None,
    "hacim": int(df["volume"].iloc[-1]) if A else None,
    ...
    "bars": bars_json(df) if A else [], "ma20": ma_json(df, 20) if A else [],
    "ma50": ma_json(df, 50) if A else [],
    "markers": [{"time": g2["iso"], "y": g2["y"]} for g2 in G["gunluk"] if g2["k"] == sym],
    "stocks": [{"k": k, "n": ISIM_SOZLUGU.get(k, ""), "kat": KATILIM_TR[katilim_etiketi(k)],
    ...
    "perf": {"kz": round(sum((g2["kz"] or 0) for g2 in kapali), 2), "karli": karli,
             "kapanan": len(kapali),
...
<style>
:root{--bg:#060D17;--card:#0C1524;--card2:#101B2E;--line:#1B2A3F;--tx:#E7EDF5;--mut:#7C8BA1;
--grn:#2EBD85;--red:#F6465D;--gold:#E4B95B}
*{box-sizing:border-box;margin:0;font-family:Inter,'Segoe UI',system-ui,sans-serif}
body{background:var(--bg);color:var(--tx)}
.top{display:flex;align-items:center;gap:16px;padding:10px 16px;background:var(--card);
...
.pill{border:1px solid var(--line);border-radius:20px;padding:5px 14px;font-size:12px;cursor:pointer}
.wrap{display:flex;min-height:860px}
.nav{width:170px;background:var(--card);border-right:1px solid var(--line);padding:12px 8px;flex-shrink:0}
.nav a{display:flex;gap:10px;align-items:center;padding:11px 12px;border-radius:10px;color:var(--mut);
...
.right{width:320px;padding:14px 14px 14px 0;flex-shrink:0}
.card{background:var(--card);border:1px solid var(--line);border-radius:14px;padding:14px;margin-bottom:14px}
.h{font-size:13px;font-weight:700;margin-bottom:10px;display:flex;align-items:center;gap:8px}
...
.sw{width:44px;height:24px;border-radius:20px;background:var(--card2);border:1px solid var(--line);
position:relative;cursor:pointer;display:inline-block;vertical-align:middle}
.sw i{position:absolute;top:2px;left:2px;width:18px;height:18px;border-radius:50%;background:var(--mut);transition:.2s}
.sw.on{background:rgba(46,189,133,.25);border-color:var(--grn)}
.sw.on i{left:22px;background:var(--grn)}
...
.warn{color:var(--mut);font-size:11px;line-height:1.5;margin-top:8px}
</style></head><body>
<div class="top">
  <div class="logo">
    <svg width="36" height="36" viewBox="0 0 44 44"><defs><linearGradient id="g" x1="0" y1="0" x2="1" y2="1">
    ...
    VERTUAL MARKET</div>
  <div style="min-width:170px"><input list="semboller" id="symsec" placeholder="Hisse ara / seç...">
  <datalist id="semboller"></datalist></div>
  <div><span class="big" id="t_fiyat"></span> <span id="t_pct" style="font-size:13px"></span>
  <div class="mut" id="t_ad"></div></div>
  <div><div class="mut">Günlük Yüksek</div><div id="t_yuksek" style="font-family:monospace"></div></div>
  <div><div class="mut">Günlük Düşük</div><div id="t_dusuk" style="font-family:monospace"></div></div>
  <div><div class="mut">Hacim</div><div id="t_hacim" style="font-family:monospace"></div></div>
  <div><div class="mut">BIST 100</div><div id="t_xu" style="font-family:monospace"></div></div>
...
<div class="wrap">
  ...
  <div class="mid">
    <div id="mesaj"></div>
    <div id="v_piyasa">
      <div class="card"><div class="h" id="grafik_baslik"></div><div id="chart" style="height:400px"></div></div>
      <div class="card"><div class="h">🤖 Bot Değerlendirmesi <span id="verdict" class="verdict"></span></div>
        <div id="yorum" style="font-size:13.5px;line-height:1.7"></div>
        <div class="warn">Kural tabanlı otomatik özet; yatırım tavsiyesi değildir. Veri ~15 dk gecikmelidir.</div></div>
      <div class="card"><div class="h">Hisse Listesi <input id="ara" placeholder="Ara..."
        style="width:180px;margin-left:auto" oninput="tabloCiz()">
        <select id="filtre" style="width:170px" onchange="tabloCiz()">
        <option value="t">Tümü</option><option value="u">◈ Katılıma Uygun</option>
        <option value="d">Katılım Dışı</option></select></div>
        <div class="scroll"><table><thead><tr><th>Kod</th><th>Şirket</th><th>Katılım</th>
        <th>Fiyat ₺</th><th>Günlük %</th></tr></thead><tbody id="tbl"></tbody></table></div></div>
    </div>
    <div id="v_alt" style="display:flex;gap:14px;flex-wrap:wrap">
      ...
      <div class="card" style="flex:1;min-width:300px"><div class="h">Bot Günlüğü</div>
        <div class="scroll" style="max-height:220px"><table><thead><tr><th>Zaman</th><th>Hisse</th>
    ...
    <div id="v_ayarlar" class="card" style="display:none"><div class="h">⚙️ Ayarlar</div>
      ...
      • AKD verisi ücretsiz kaynaklarda yayınlanmaz.<br>
  ...
  <div class="right">
    <div class="card"><div class="h">🤖 Otomatik Al-Sat Botu <span class="mut">(SANAL)</span></div>
      ...
      "Şimdi Tara" ile çalışır; 7/24 kendi başına çalışması sunucu kurulumu gerektirir.</div></div>
...
<script>
const D = __DATA__;
function go(p){const q=new URLSearchParams();q.set("sym",D.sym);q.set("view",D.view);
for(const k in p){if(p[k]===null)q.delete(k);else q.set(k,p[k]);}
window.parent.location.search=q.toString();}
function nav(v){document.querySelectorAll(".nav a").forEach(a=>a.classList.toggle("on",a.dataset.v===v));
const piyasa=(v==="piyasa"||v==="bot");
document.getElementById("v_piyasa").style.display=piyasa?"":"none";
document.getElementById("v_alt").style.display=(v==="piyasa"||v==="portfoy"||v==="bot")?"flex":"none";
document.getElementById("v_islemler").style.display=v==="islemler"?"":"none";
document.getElementById("v_ayarlar").style.display=v==="ayarlar"?"":"none";
D.view=v;}
...
document.getElementById("t_ad").textContent=D.sym+" · "+(D.isim||"")+" · "+D.kat;
...
// sembol seçici
const dl=document.getElementById("semboller");
D.stocks.forEach(s=>{const o=document.createElement("option");o.value=s.k;o.label=s.n;dl.appendChild(o);});
const si=document.getElementById("symsec");
si.addEventListener("change",()=>{const v=si.value.toUpperCase().trim();
if(D.stocks.some(s=>s.k===v)&&v!==D.sym)go({sym:v});});
// karar
const vd=document.getElementById("verdict");vd.textContent=D.karar+(D.skor!=null?` · ${D.skor}/100`:"");
vd.style.background=D.karar.includes("AL")&&!D.karar.includes("SAT")?"rgba(46,189,133,.15)":
D.karar.includes("SAT")?"rgba(246,70,93,.15)":"rgba(124,139,161,.15)";
vd.style.color=D.karar.includes("AL")&&!D.karar.includes("SAT")?"#2EBD85":
D.karar.includes("SAT")?"#F6465D":"#7C8BA1";
document.getElementById("yorum").textContent=D.yorum;
document.getElementById("grafik_baslik").textContent=D.sym+" · Günlük Grafik · Destek "
...
layout:{background:{color:"#0C1524"},textColor:"#7C8BA1"},
grid:{vertLines:{color:"#101B2E"},horzLines:{color:"#101B2E"}},height:400,
timeScale:{borderColor:"#1B2A3F"},rightPriceScale:{borderColor:"#1B2A3F"}});
...
vol.setData(D.bars.map(b=>({time:b.time,value:b.volume,
color:b.close>=b.open?"rgba(46,189,133,.45)":"rgba(246,70,93,.45)"})));
ch.addLineSeries({color:"#2EBD85",lineWidth:1}).setData(D.ma20);
ch.addLineSeries({color:"#F6465D",lineWidth:1}).setData(D.ma50);
if(D.destek)cs.createPriceLine({price:D.destek,color:"#2EBD85",lineStyle:2,title:"DESTEK"});
...
else document.getElementById("chart").innerHTML="<div class='mut' style='padding:40px;text-align:center'>Grafik verisi yok.</div>";
// hisse tablosu
...
["logtbl","logtbl2"].forEach((id,full)=>{const el=document.getElementById(id);
