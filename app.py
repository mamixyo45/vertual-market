...
# BIST Katılım Tüm Endeksi (XKTUM) resmî bileşen listesi
# Kaynak: Borsa İstanbul duyurusu · Geçerlilik: 1 Mayıs – 30 Eylül 2026 · 237 hisse
KATILIM_DONEMI = "1 Mayıs – 30 Eylül 2026"
KATILIM_XKTUM = set("""AAGYO ACSEL AHGAZ AHSGY AKFYE AKHAN ALBRK ALCTL ALFAS ALKA ALKIM ALKLC
ALTNY ALVES ANGEN ARASE ARDYZ ARENA ARFYE ASELS ATAKP ATATP AVPGY AYEN BAHKM BAKAB BASGZ BAYRK
BEGYO BERA BESTE BIENY BIMAS BINBN BINHO BMSTL BORSK BOSSA BRISA BRKSN BRLSM BSOKE BUCIM BURCE
BURVA BYDNR CANTE CATES CELHA CEMTS CEMZY CIMSA CMBTN COSMO CVKMD CWENE DAPGM DARDL DCTTR DENGE
DESPC DGATE DITAS DMSAS DNISI DOFER DOFRB DOGUB DYOBY EBEBK EDATA EDIP EFOR EGEPO EGGUB EGPRO
EKGYO EKSUN ELITE EMPAE ENJSA EREGL ESCOM EUPWR EYGYO FADE FONET FORMT FORTE FRMPL FZLGY GEDZA
GENIL GENKM GENTS GEREL GESAN GLRMK GMTAS GOKNR GOLTS GOODY GRSEL GRTHO GUBRF GUNDG HATSN HKTM
HOROZ HRKET IDGYO IHEVA IHLAS IHLGM IHYAY IMASM INGRM INTEM ISDMR IZFAS IZINV JANTS KARSN KATMR
KBORU KCAER KIMMR KLSER KLSYN KMPUR KNFRT KOCMT KONKA KONYA KOPOL KOTON KRDMA KRDMB KRDMD KRGYO
KRONT KRPLS KRSTL KRVGD KTLEV KUTPO KZBGY LKMNH LMKDC LOGO LXGYO MAGEN MAKIM MARBL MAVI MCARD
MEDTR MEGMT MEKAG MERCN MERKO MEYSU MNDTR MOBTL MOPAS MPARK NETAS NETCD NTGAZ OBAMS OBASE ONCSM
ORGE OSTIM OZATD OZGYO OZRDN OZYSR PAGYO PARSN PASEU PENGD PENTA PETKM PKART PLTUR PNSUT POLHO
PRKME QUAGR RALYH RGYAS RNPOL RODRG RUBNS SAFKR SAMAT SANEL SANKO SARKY SAYAS SDTTR SEKUR SELEC
SELVA SILVR SMART SNGYO SNICA SOKE SRVGY SUNTK SURGY SUWEN TARKM TEZOL TKFEN TKNSA TMPOL TUCLK
TUKAS TUPRS TUREX TURGG UCAYM UFUK ULUSE USAK VAKKO VANGD YATAS YEOTK YIGIT YKSLN YUNSA ZEDUR
ZERGY""".split())
...
def katilim_etiketi(kod):
    """Resmî XKTUM bileşen listesine göre: listede olan uygun, olmayan değildir."""
    return "uygun" if kod in KATILIM_XKTUM else "disi"
...
def toplu_veri(kodlar):
    """Tüm hisselerin 6 aylık kapanışları — Yahoo spark uç noktası
    (100 sembol/istek, toplam ~7 hafif istek; fiyat, %, hızlı AI kararı)."""
    out = {}
    kodlar = list(kodlar)
    for i in range(0, len(kodlar), 100):
        ...
        try:
            resp = requests.get(
                ...
                params={"symbols": ",".join(k + ".IS" for k in parca),
                        "range": "6mo", "interval": "1d"},
                headers=UA, timeout=20)
            resp.raise_for_status()
            sonuc = resp.json().get("spark", {}).get("result", []) or []
        except Exception:
            continue
        for kayit in sonuc:
            try:
                k = kayit["symbol"].replace(".IS", "")
                q = kayit["response"][0]["indicators"]["quote"][0]["close"]
                c = pd.Series([x for x in q if x is not None], dtype=float)
                if len(c) < 2:
                    continue
                son, once = float(c.iloc[-1]), float(c.iloc[-2])
                sk = 50
                if len(c) >= 55:
                    sk += 12 if float(c.rolling(20).mean().iloc[-1]) > float(c.rolling(50).mean().iloc[-1]) else -12
                if len(c) >= 16:
                    d = c.diff()
                ...
                out[k] = {"f": round(son, 2), "p": round((son - once) / once * 100, 2),
                          "h": None, "ai": karar_ver(max(5, min(95, sk)))}
            except Exception:
                continue
    # hacim: yalnız bilinen ~100 hisse için tek toplu istek (hafif)
    try:
        dfh = yf.download([k + ".IS" for k in ISIM_SOZLUGU], period="5d", interval="1d",
                          auto_adjust=True, progress=False, group_by="ticker", threads=True)
        for k in ISIM_SOZLUGU:
            try:
                v = dfh[k + ".IS"]["Volume"].dropna()
                if len(v) and k in out:
                    out[k]["h"] = int(v.iloc[-1])
            except Exception:
                continue
    except Exception:
        pass
...
def temel_doldur(kodlar):
    ...
    for k in kodlar:
        if k in depo:
            continue
        if sayac >= 8 or _time.time() - basla > 7:
            break
        try:
...
def endeksler():
    out = []
    for ad, semboller in [("BIST 100", ["XU100.IS"]), ("BIST 30", ["XU030.IS"]),
                          ("BIST KATILIM", ["XKTUM.IS", "XK100.IS", "XK030.IS"])]:
        for s in semboller:
            try:
...
def haberler():
        ...
        try:
            ...
            for item in list(root.iter("item"))[:4]:
                ...
                try:
                    ...
                    saat = ""
                if baslik:
                    hepsi.append({"b": baslik, "l": link, "s": saat, "k": ad})
        except Exception:
            pass
        return hepsi
    ...
    with ThreadPoolExecutor(max_workers=7) as ex:
        for grup in ex.map(oku, kaynaklar):
            out.extend(grup)
    return out[:20]
...
def tf_paket(sym):
    """Seçili hissenin 7 zaman dilimini paralel indirir."""
    def indir(tanim):
        key, ad, per, itv, kural = tanim
        try:
            d = yf.download(f"{sym}.IS", period=per, interval=itv,
                            auto_adjust=True, progress=False)
            ...
            return key, (ad, d, itv, kural)
        except Exception:
            return key, None

    with ThreadPoolExecutor(max_workers=7) as ex:
        sonuc = dict(ex.map(indir, TFDEF))
    return sonuc

DURUM_YOLU = "/tmp/vm_state.json"
...
def durum():
    try:
        with open(DURUM_YOLU, encoding="utf-8") as f:
            d = json.load(f)
        for k in ("bakiye", "baslangic", "poz", "gunluk", "izleme", "risk", "tutar", "aktif"):
            if k not in d:
                raise ValueError
        return d
    except Exception:
...
LOGO = """<svg width="118" height="64" viewBox="0 0 200 104" style="filter:drop-shadow(0 3px 12px rgba(228,185,91,.4))">
<defs><linearGradient id="au" x1="0" y1="0" x2="0" y2="1">
<stop offset="0" stop-color="#FBEDC2"/><stop offset=".45" stop-color="#E4B95B"/>
<stop offset="1" stop-color="#9C731F"/></linearGradient></defs>
<g stroke="url(#au)" stroke-width="11" fill="none" stroke-linecap="round" stroke-linejoin="round">
<path d="M10 18 L42 90 L74 18"/>
<path d="M86 90 L86 22 L114 64 L144 12"/>
<path d="M152 26 L174 80"/></g>
<polygon points="132,18 148,3 155,24" fill="url(#au)"/>
<polygon points="162,74 179,94 186,67" fill="url(#au)"/>
<g stroke="#FFF6DC" stroke-width="2.5" fill="none" stroke-linecap="round" opacity=".45">
<path d="M12 19 L42 86"/><path d="M88 86 L88 24"/></g>
<text x="42" y="50" font-family="Georgia,serif" font-size="17" font-weight="700"
 fill="url(#au)" text-anchor="middle">$</text>
<text x="128" y="58" font-family="Georgia,serif" font-size="16" font-weight="700"
 fill="url(#au)" text-anchor="middle">₺</text></svg>"""
...
try:
    edx = {e["ad"]: e for e in endeksler()}
except Exception:
    edx = {}
def _edx_kutu(ad):
    e = edx.get(ad)
    if e:
        deger = f"{e['v']:,.0f}".replace(",", ".")
        alt = pct_span(e["p"])
    else:
        deger, alt = "—", "<span style='color:#8496AD;font-size:12px'>yükleniyor</span>"
    return (f"<div style='text-align:right;min-width:96px'>"
            f"<div style='color:#8496AD;font-size:11px;letter-spacing:1px'>{ad}</div>"
            f"<div style='font-family:monospace;font-weight:700;font-size:15px'>{deger}</div>{alt}</div>")
edx_html = "".join(_edx_kutu(ad) for ad in ("BIST 100", "BIST 30", "BIST KATILIM"))
st.markdown(
...
with solc:
    view = st.radio("Menü", ["📈 Piyasa", "🎮 Sanal Borsa", "🤖 Bot", "💼 Portföy",
                             "🧾 İşlemler", "⚙️ Ayarlar"],
                    label_visibility="collapsed")
view = {"📈 Piyasa": "piyasa", "🎮 Sanal Borsa": "oyun", "🤖 Bot": "bot",
        "💼 Portföy": "portfoy", "🧾 İşlemler": "islemler", "⚙️ Ayarlar": "ayarlar"}[view]
...
with sagc:
    ...
    A = None
    dfb = None
    if view in ("piyasa", "bot", "portfoy", "oyun"):
        try:
                ...
                if view == "piyasa":
                    ...
                    for key, ad, per, itv, kural in TFDEF:
                        ...
                        if not s:
                            continue
                        _, d, itv2, kural2 = s
                        unix = itv2 in ("15m", "1h") and kural2 != "YE"
                        tfdata[key] = {"ad": ad, "bars": bars_json(d, unix),
                                       "ma20": ma_json(d, 20, unix),
                                       "ma50": ma_json(d, 50, unix)}
                        tford.append(key)
        except Exception as e:
            st.error(f"Veri yüklenirken sorun oluştu ({type(e).__name__}); sayfayı yenileyin.")
        A = analiz(dfb) if (dfb is not None and len(dfb) >= 30) else None

    # ---------------- 🎮 SANAL BORSA OYUNU ----------------
    if view == "oyun":
        HEDEFLER = [12000, 15000, 20000, 27000, 36000, 50000, 70000, 100000]
        if "oyun" not in st.session_state:
            st.session_state["oyun"] = {"bakiye": 10000.0, "poz": {}, "gecmis": []}
        O = st.session_state["oyun"]

        def opy(k):
            return piyasa.get(k, {}).get("f")

        poz_deg = sum(p["adet"] * (opy(k) or p["giris"]) for k, p in O["poz"].items())
        toplam_o = O["bakiye"] + poz_deg
        sev = 1
        for hdf in HEDEFLER:
            if toplam_o >= hdf:
                sev += 1
        sev = min(sev, len(HEDEFLER))
        hedef_o = HEDEFLER[sev - 1]

        st.markdown("### 🎮 Sanal Borsa — gerçek hisseler, sanal 10.000 ₺")
        st.caption("Fiyatlar gerçek BIST verisidir (~15 dk gecikmeli). Tamamen sanal paradır; "
                   "yatırım tavsiyesi değildir. İlerleme bu tarayıcı oturumunda saklanır.")

        # Seviye haritası — oyuna başlamadan önce tüm yol görünür
        harita = []
        for i, hdf in enumerate(HEDEFLER, start=1):
            if toplam_o >= hdf:
                harita.append(f"✅ **S{i}**<br>{hdf/1000:.0f}b₺")
            elif i == sev:
                harita.append(f"🔶 **S{i}**<br>{hdf/1000:.0f}b₺")
            else:
                harita.append(f"🔒 S{i}<br>{hdf/1000:.0f}b₺")
        st.markdown(
            ...
            + "".join(f"<div style='flex:1;min-width:64px;background:#0C1524;border:1px solid "
                      f"{'#E4B95B' if ('🔶' in h) else '#1B2A3F'};border-radius:10px;"
                      f"padding:8px 4px;font-size:12px'>{h}</div>" for h in harita)
            + "</div>", unsafe_allow_html=True)

        m1, m2, m3 = st.columns(3)
        m1.metric("Toplam Varlık", f"{toplam_o:,.0f} ₺".replace(",", "."))
        m2.metric("Nakit", f"{O['bakiye']:,.0f} ₺".replace(",", "."))
        m3.metric(f"Seviye {sev} Hedefi", f"{hedef_o:,.0f} ₺".replace(",", "."))
        st.progress(min(1.0, toplam_o / hedef_o))

        a1, a2, a3 = st.columns([2.2, 1, 1])
        osym = a1.selectbox("Hisse", tum_kodlar,
                            format_func=lambda k: f"{k} — {ISIM_SOZLUGU.get(k, '')}".rstrip(" —"),
                            key="osym")
        ofi = opy(osym)
        a2.metric("Fiyat", "—" if ofi is None else f"{ofi:.2f} ₺")
        otutar = a3.number_input("Tutar ₺", 100.0, 1000000.0, 1000.0, step=100.0, key="otutar")
        if st.button(f"🟢 {osym} AL", use_container_width=True, type="primary"):
            if ofi is None:
                st.warning("Bu hissenin fiyatı şu an alınamıyor.")
            else:
                adet = int(min(otutar, O["bakiye"]) // ofi)
                if adet < 1:
                    st.warning("Yetersiz bakiye.")
                else:
                    O["bakiye"] -= adet * ofi
                    p = O["poz"].get(osym, {"adet": 0, "giris": 0.0})
                    yeni_adet = p["adet"] + adet
                    p["giris"] = (p["giris"] * p["adet"] + ofi * adet) / yeni_adet
                    p["adet"] = yeni_adet
                    O["poz"][osym] = p
                    O["gecmis"].insert(0, f"🟢 AL · {osym} · {adet} lot @ {ofi:.2f} ₺")
                    st.success(f"{adet} lot {osym} alındı.")
                    st.rerun()

        if O["poz"]:
            st.markdown("**Açık Pozisyonlar**")
            for k, p in list(O["poz"].items()):
                g = opy(k) or p["giris"]
                kz = (g - p["giris"]) * p["adet"]
                c1, c2, c3, c4 = st.columns([1.6, 1.6, 1.6, 1])
                c1.write(f"**{k}** · {p['adet']} lot")
                c2.write(f"Giriş {p['giris']:.2f} → {g:.2f} ₺")
                c3.markdown(f"<span style='color:{'#2EBD85' if kz >= 0 else '#F6465D'}'>"
                            f"{'+' if kz >= 0 else ''}{kz:,.0f} ₺</span>".replace(",", "."),
                            unsafe_allow_html=True)
                if c4.button("SAT", key=f"osat_{k}", use_container_width=True):
                    O["bakiye"] += p["adet"] * g
                    O["gecmis"].insert(0,
                        f"🔴 SAT · {k} · {p['adet']} lot @ {g:.2f} ₺ · K/Z "
                        f"{'+' if kz >= 0 else ''}{kz:,.0f} ₺".replace(",", "."))
                    del O["poz"][k]
                    st.rerun()
        else:
            st.caption("Henüz açık pozisyon yok — yukarıdan ilk alımını yap.")

        if O["gecmis"]:
            with st.expander("🧾 İşlem Geçmişi"):
                for satir in O["gecmis"][:30]:
                    st.write(satir)
        if st.button("♻️ Oyunu Sıfırla (10.000 ₺)"):
            st.session_state["oyun"] = {"bakiye": 10000.0, "poz": {}, "gecmis": []}
            st.rerun()
        st.stop()

    temel = dict(temel_depo()) if view == "piyasa" else {}
  ...
  <div class="card">
    ...
    <div class="h" style="margin-top:2px">
      ...
      <span style="margin-left:auto;display:flex;gap:5px;align-items:center">
        ...
        <button class="btn tfb" id="d_sil">🗑 Temizle</button>
        <button class="btn tfb" id="d_fit">⤢ Sığdır</button></span></div>
    <div id="chart_p" style="height:410px"></div>
    <div class="warn">✏️ Trend: grafikte iki noktaya dokun · ─ Yatay: bir noktaya dokun ·
    Zaman dilimi değişince çizimler temizlenir. Grafiği parmakla kaydırabilir, iki parmakla
        yakınlaştırıp uzaklaştırabilirsin; ⤢ Sığdır tümünü ekrana getirir.</div></div>
...
<div id="v_ayarlar" class="card" style="__D_ayarlar__"><div class="h">⚙️ Bilgi</div>
  ...
  • Katılım etiketleri Borsa İstanbul'un resmî XKTUM bileşen listesine dayanır (dönem: 1 Mayıs – 30 Eylül 2026; her endeks döneminde güncellenmelidir).<br>
...
document.getElementById("d_fit").addEventListener("click",()=>{if(GP)GP.ch.timeScale().fitContent();});
...
document.getElementById("a_izleme").textContent=D.ayar.izleme.join(", ")||"—";}
    ...
    if view == "piyasa":
        try:
            temel_doldur(list(ISIM_SOZLUGU) + [k for k in tum_kodlar if k not in ISIM_SOZLUGU])
        except Exception:
            pass
