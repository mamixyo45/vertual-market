...
def toplu_veri(kodlar):
    ...
    out = {}
    kodlar = list(kodlar)
    parcalar = [kodlar[i:i + 100] for i in range(0, len(kodlar), 100)]

    def indir(parca):
        try:
            return parca, yf.download([k + ".IS" for k in parca], period="6mo",
    ...
    with ThreadPoolExecutor(max_workers=4) as ex:
        sonuclar = list(ex.map(indir, parcalar))
    for parca, df in sonuclar:
        if df is None:
            continue
        for k in parca:
            try:
...
LOGO = """<svg width="96" height="62" viewBox="0 0 150 96" style="filter:drop-shadow(0 3px 12px rgba(228,185,91,.35))">
<defs><linearGradient id="au" x1="0" y1="0" x2="0" y2="1">
<stop offset="0" stop-color="#F9E9B8"/><stop offset=".45" stop-color="#E4B95B"/>
<stop offset="1" stop-color="#A97C24"/></linearGradient></defs>
<path d="M10 22 L34 82 L58 22" stroke="url(#au)" stroke-width="10" fill="none"
 stroke-linecap="round" stroke-linejoin="round"/>
<path d="M66 82 L66 26 L88 62 L106 20" stroke="url(#au)" stroke-width="10" fill="none"
 stroke-linecap="round" stroke-linejoin="round"/>
<polygon points="96,22 108,6 116,24" fill="url(#au)"/>
<path d="M112 30 L128 74" stroke="url(#au)" stroke-width="10" fill="none" stroke-linecap="round"/>
<polygon points="118,72 132,88 138,66" fill="url(#au)"/>
<text x="46" y="46" font-family="Georgia,serif" font-size="17" font-weight="700"
